...
class AuthService:
    ...
    def get_user_by_code(self, code: str) -> User | None:
    ...
    def google_login(self, code: str, redirect_uri: str) -> Token:
        # 구글 액세스 토큰 요청
        response = httpx.post(
        ...
        if not google_user.verified_email:
            raise HTTPException(
                status_code=401, detail="Google account is not verified"
            )
        ...
        if user and user.deleted_at:
            raise HTTPException(
                status_code=401, detail="Cannot sign in with this email"
            )
        ...
        return Token(
            access_token=create_access_token(
                user.code, expires_delta=timedelta(days=1)
            ),
            token_type="Bearer",
        )

    def apple_login(self, code: str, redirect_uri: str) -> Token:
